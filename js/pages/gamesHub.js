  ...
  container.innerHTML = `
    <div class="games-hub-container" style="max-width: 800px; margin: 0 auto; padding: 20px;">
      ...
      <div class="games-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px;">
        ${games.map(g => {
          const status = Storage.getGameLevelStatus(g.id);
          const l2Unlocked = status.level2 && status.level2.unlocked;
          const l3Unlocked = status.level3 && status.level3.unlocked;
          return `
